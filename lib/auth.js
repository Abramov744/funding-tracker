...
// than only in memory so a process restart doesn't lose the count; survives
// a Railway *redeploy* too as long as a persistent Volume is mounted (see
// dataDir.js) — without one it's still just this container's filesystem.
const GUEST_LOG_PATH = path.join(dataDir(), 'guest-logins.json');
