...
const state = {
  rankBySymbol: new Map(), // lowercase base symbol -> { rank, name }
  updatedAt: null,
};
  ...
  for (let page = 1; page <= PAGES; page++) {
    ...
    for (const coin of coins) {
      ...
      if (!existing || coin.market_cap_rank < existing.rank) {
        rankBySymbol.set(symbol, { rank: coin.market_cap_rank, name: coin.name });
      }
    }
...
module.exports = { lookup, startAutoRefresh, refresh, state };
