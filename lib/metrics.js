...
const QUOTE_SUFFIXES = ['_USDT', '_USDC', 'USDT', 'USDC'];

// "BTC_USDT" / "BTCUSDT" -> "BTC"  (lets the UI compare/search the same coin across exchanges)
function baseAsset(symbol) {
  for (const suf of QUOTE_SUFFIXES) {
    if (symbol.endsWith(suf)) return symbol.slice(0, -suf.length);
  }
  return symbol;
}
