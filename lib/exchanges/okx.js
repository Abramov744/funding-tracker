...
async function fetchCurrent() {
  const [instruments, marks, openInterest] = await Promise.all([
    getJson('/api/v5/public/instruments?instType=SWAP'),
    getJson('/api/v5/public/mark-price?instType=SWAP'),
    getJson('/api/v5/public/open-interest?instType=SWAP'),
  ]);
  ...
  // oiUsd is already computed in USD by OKX itself — confirmed on a real
  // instType=SWAP dump (e.g. BTC-USDT-SWAP: oi=2,940,318.16 contracts,
  // oiCcy=29,403.18 BTC, oiUsd=2,483,131,029.62 — oiCcy * markPrice matches
  // oiUsd, so oiUsd needs no further conversion, unlike every other exchange
  // integrated so far). One bulk call for every instrument, not per-symbol.
  const oiUsdByInst = new Map(openInterest.map((o) => [o.instId, Number(o.oiUsd)]));
  ...
  const results = await mapWithConcurrency(swaps, CONCURRENCY, async (inst) => {
    ...
    return {
      ...
      openInterestUsd: oiUsdByInst.get(inst.instId) ?? null,
    };
  });
