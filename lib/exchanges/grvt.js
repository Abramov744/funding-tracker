...
// base ticker -> full instrument string ("BTC" -> "BTC_USDT_Perp"), filled by
// fetchCurrent and read by fetchOpenInterest, which needs the full instrument
// identifier for /full/v1/ticker — unlike fetchHistory, which is already
// cached by base ticker and needs no lookup.
let instrumentByBase = new Map();
...
async function fetchCurrent() {
  ...
  const results = await mapWithConcurrency(candidates, CONCURRENCY, async (inst) => {
    ...
    instrumentByBase.set(inst.base, inst.instrument);
...
// Unlike fetchHistory, this isn't cached by fetchCurrent — /full/v1/ticker is
// a separate endpoint from /full/v1/funding, called once per history
// candidate via cache.js's fetchOpenInterest hook (same cost model already
// paid for fetchHistory). open_interest is in base-asset units, not USD —
// confirmed on a real BTC sample: open_interest=2516.158845178 *
// mark_price=$85,829.26 ≈ $216.0M, plausible for GRVT's scale (similar
// order of magnitude to Lighter's $186.7M, #58).
async function fetchOpenInterest(symbol, price) {
  const instrument = instrumentByBase.get(symbol);
  if (!instrument || !price) return null;
  const ticker = await postJson('/full/v1/ticker', { instrument });
  const oi = Number(ticker && ticker.open_interest);
  return Number.isFinite(oi) ? oi * price : null;
}

module.exports = { id: 'grvt', label: 'GRVT', fetchCurrent, fetchHistory, fetchOpenInterest };
