...
async function fetchCurrent() {
  ...
  return (prices || [])
    ...
    .map((p) => {
      const price = Number(p.mark) || Number(p.oracle) || null;
      // open_interest is in base-asset units, not USD — confirmed on a real
      // BTC sample: open_interest=437.67441 * mark=85,206.999 ≈ $37.3M,
      // consistent with volume_24h ($268.25M) at Pacifica's scale, same
      // pattern as Paradex/Hyperliquid/Lighter/GRVT.
      const openInterestUsd = price && p.open_interest !== undefined ? Number(p.open_interest) * price : null;
      return {
        exchange: 'pacifica',
