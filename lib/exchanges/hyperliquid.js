...
async function fetchCurrent() {
  ...
  return universe
    .map((asset, i) => {
      ...
      const price = Number(ctx.markPx) || null;
      // openInterest is in base-asset units, not USD — confirmed on a real BTC
      // sample: openInterest=42,567.6507 * markPx=84,590.0 ≈ $3.6B, consistent
      // with dayNtlVlm ($3.21B) at a nearby average intraday price. Same
      // pattern as Paradex's own base-asset-denominated open_interest field.
      const openInterestUsd = price && ctx.openInterest !== undefined ? Number(ctx.openInterest) * price : null;
      return {
