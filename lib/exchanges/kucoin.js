...
async function fetchCurrent() {
  ...
  return (contracts || [])
    ...
    .map((c) => ({
      ...
      price: Number(c.markPrice) || null,
      // openInterest is a raw contract count, not a base-asset quantity —
      // multiplier converts contracts to base-asset units for these quanto
      // USDT-margined contracts (isQuanto: true, isInverse: false for all of
      // them, since only quoteCurrency==="USDT" ones are kept above). Confirmed
      // on a real XBTUSDTM sample: 9,546,884 contracts * 0.001 BTC/contract *
      // $84,186.26 mark price ≈ $803.7M, a plausible BTC OI figure for KuCoin.
      openInterestUsd:
        c.multiplier && c.markPrice ? Number(c.openInterest) * Number(c.multiplier) * Number(c.markPrice) : null,
    }));
}
