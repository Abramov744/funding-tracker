...
async function fetchCurrent() {
  ...
  return markets
    ...
    .map((m) => {
      const price = Number(m.oraclePrice) || null;
      // openInterest is in base-asset units, not USD (there's also a
      // baseOpenInterest field with a different value — confirmed against
      // dYdX's own BTC-USD market page, which showed 188.26, matching
      // openInterest=188.2719, not baseOpenInterest=782.0931).
      const openInterestUsd = price && m.openInterest !== undefined ? Number(m.openInterest) * price : null;
      return {
        exchange: 'dydx',
        symbol: m.ticker,
...
// ticker is a path segment here, not a query param (?ticker=... 404s).
async function fetchHistory(symbol, limit = 200) {
  const data = await getJson(`/historicalFunding/${encodeURIComponent(symbol)}?limit=${limit}`);
