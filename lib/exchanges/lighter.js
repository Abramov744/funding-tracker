...
async function fetchCurrent() {
  ...
  const rows = await mapWithConcurrency(crypto, CONCURRENCY, async (m) => {
    ...
    const price = Number(m.mark_price) || Number(m.last_trade_price) || null;
    // open_interest is already in base-asset units — every market's own
    // `multiplier` field in a real dump is 1.0, so no conversion is needed.
    // Confirmed on a real BTC sample: open_interest=2207.42546 *
    // mark_price=84,584.5 ≈ $186.7M, plausible for Lighter's scale.
    const openInterestUsd = price && m.open_interest !== undefined ? Number(m.open_interest) * price : null;
