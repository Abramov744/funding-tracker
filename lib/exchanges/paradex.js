...
async function fetchCurrent() {
  ...
  const rows = summaries
    ...
    .map((s) => {
      ...
      return {
        ...
        // open_interest is in base-asset units (e.g. "49.96874" BTC), not USD —
        // confirmed by cross-checking against mark_price on a real dump: taking
        // it as-is put ETH's "OI" ~30x above BTC's (1539 vs 49.97), which doesn't
        // match either coin's actual relative market size; multiplying by price
        // instead gives BTC ~$3.8M and ETH ~$3.7M, consistent with each other and
        // with SOL/DOGE/SUI scaling down sensibly below them.
  ...
  return top.map((r) => ({
    ...
    price: r.price,
    openInterestUsd: r.price ? r.openInterest * r.price : null,
  }));
}
